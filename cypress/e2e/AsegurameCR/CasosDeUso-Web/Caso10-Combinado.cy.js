...
describe("Caso 10: Cotizar seguro enfermedades graves con el url del aliado", () => {
  ...
  it.only("Sistema administrativo, modulo de aliados", () => {
    cy.visit(Cypress.env("URL_ASUGURAMECR"));
    ...
    cy.get(':nth-child(1) > :nth-child(1) > .grid-iniciales > .MuiTypography-root').then($value => {
      cy.readFile("archivo.json").then(function (data) {
        cy.writeFile("archivo.json", { ...data, nombreAliado: $value.text()})
    }); 
    })
    administrativoCR.clickCopyUrl(1)
